    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" onClick={closeDialog} />
            ...
            {success ? (
              <div className="flex flex-col items-center justify-center py-6 text-center animate-fade-in">
                ...
                <p className="mt-3 text-sm text-muted leading-relaxed">
                  Thank you for booking the <strong>{packageName}</strong>. Our diagnostic team will contact you shortly to confirm the appointment details.
                </p>
                <button className="btn btn-primary mt-8 w-full" onClick={closeDialog}>
                  Done
                </button>
              </div>
            ) : (
              <div className="animate-fade-in">
                <h2 className="text-xl font-bold text-foreground">Book {packageName}</h2>
                <div className="mt-4 border-b border-line pb-4 mb-5">
                  <div className="flex justify-between items-center font-medium">
                    <span className="text-sm text-muted">Total Payable at Clinic</span>
                    <span className="text-xl text-primary">₹{packagePrice}</span>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <option value="023058ef-ada0-4ac2-9eaf-7033e318a585">{t("tramDepot")} (Shibpur)</option>
                        <option value="03476cf4-170c-4f06-a1a8-f95620f52b2e">{t("vivekVihar")}</option>
                        <option value="ef40552e-31ad-4bea-bd44-c293fef56ed0">{t("pilkhana")}</option>
                      </select>
                      <svg className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted pointer-events-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
                    </div>
                  </div>
