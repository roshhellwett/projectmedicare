...
describe("Bulletins (Products & Offers) Integration Tests", () => {
  ...
  beforeAll(async () => {
    // 1. Create a Product
    const pRes = await adminSupabase.from("bulletins").insert({
      body: "Test Product",
      kind: "product",
      pinned: true
    }).select().single();
    ...
    // 2. Create an Offer
    const oRes = await adminSupabase.from("bulletins").insert({
      body: "Test Offer",
      kind: "offer",
  ...
  afterAll(async () => {
    if (testProduct) await adminSupabase.from("bulletins").delete().eq("id", testProduct.id);
    if (testOffer) await adminSupabase.from("bulletins").delete().eq("id", testOffer.id);
  });
