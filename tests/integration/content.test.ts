...
describe("Content Tables Tests (Camp Posts & Bulletins)", () => {
  ...
  beforeAll(async () => {
    ...
    const c2 = await adminSupabase.from("camp_posts").insert({
      title: "Inactive Camp",
      description: "Desc",
      venue: "Venue",
      address: "Address",
    ...
    const b1 = await adminSupabase.from("bulletins").insert({
    ...
    const b2 = await adminSupabase.from("bulletins").insert({
  ...
  afterAll(async () => {
    if (activeCamp) await adminSupabase.from("camp_posts").delete().eq("id", activeCamp.id);
    if (inactiveCamp) await adminSupabase.from("camp_posts").delete().eq("id", inactiveCamp.id);
    if (visibleBulletin) await adminSupabase.from("bulletins").delete().eq("id", visibleBulletin.id);
    if (expiredBulletin) await adminSupabase.from("bulletins").delete().eq("id", expiredBulletin.id);
  });
  ...
  describe("Camp Posts", () => {
    ...
    it("should allow public to read ONLY active camp posts", async () => {
      const { data, error } = await anonSupabase.from("camp_posts").select("id").in("id", [activeCamp.id, inactiveCamp.id]);
  ...
  describe("Bulletins", () => {
    it("should allow public to read ONLY visible bulletins", async () => {
      const { data, error } = await anonSupabase.from("bulletins").select("id").in("id", [visibleBulletin.id, expiredBulletin.id]);
      expect(error).toBeNull();
