...
describe("Database Integration Tests", () => {
  ...
  describe("Package Orders Race Condition", () => {
    ...
    beforeAll(async () => {
      ...
      const s1 = await supabase.from("pharmacy_stores").insert({
        name: "Test Store 1",
        address: "123 Test St",
        phone: "1111111111",
        lat: 0,
      ...
      const s2 = await supabase.from("pharmacy_stores").insert({
        name: "Test Store 2",
      ...
      const p = await supabase.from("packages").insert({
        name: "Test Package",
        description: "Test",
        tests: ["A"],
        market_price: 100,
    ...
    afterAll(async () => {
      // Cleanup
      if (testOrder) await supabase.from("package_orders").delete().eq("id", testOrder.id);
      if (testPackage) await supabase.from("packages").delete().eq("id", testPackage.id);
      if (testStore1) await supabase.from("pharmacy_stores").delete().eq("id", testStore1.id);
      if (testStore2) await supabase.from("pharmacy_stores").delete().eq("id", testStore2.id);
    });

    it("should allow only one store to claim an order concurrently", async () => {
      // 1. Insert a pending order (no store_id)
      const orderRes = await supabase.from("package_orders").insert({
      ...
      const finalOrder = await supabase.from("package_orders").select("store_id").eq("id", testOrder.id).single();
