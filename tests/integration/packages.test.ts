...
describe("Packages Integration Tests", () => {
  ...
  beforeAll(async () => {
    // 1. Create a Package
    const pRes = await adminSupabase.from("packages").insert({
    ...
    // 2. Create a Store
    const sRes = await adminSupabase.from("pharmacy_stores").insert({
  ...
  afterAll(async () => {
    if (testOrder) await adminSupabase.from("package_orders").delete().eq("id", testOrder.id);
    if (testPackage) await adminSupabase.from("packages").delete().eq("id", testPackage.id);
    if (testStore) await adminSupabase.from("pharmacy_stores").delete().eq("id", testStore.id);
  });

  it("should allow public to read packages", async () => {
    const { data, error } = await anonSupabase.from("packages").select("*").eq("id", testPackage.id);
