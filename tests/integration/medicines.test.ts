...
describe("Medicines and Patient Rates Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const medRes = await adminSupabase.from("medicines").insert({
      s_no: 999,
    ...
    const rateRes = await adminSupabase.from("patient_rates").insert({
      sl_no: 999,
  ...
  afterAll(async () => {
    if (testMedicine) await adminSupabase.from("medicines").delete().eq("id", testMedicine.id);
    if (testPatientRate) await adminSupabase.from("patient_rates").delete().eq("id", testPatientRate.id);
  });

  describe("Medicines Table", () => {
    it("should allow anon users to read medicines", async () => {
      const { data, error } = await anonSupabase.from("medicines").select("*").eq("id", testMedicine.id);
  ...
  describe("Patient Rates Table", () => {
    it("should allow anon users to read patient rates", async () => {
      const { data, error } = await anonSupabase.from("patient_rates").select("*").eq("id", testPatientRate.id);
      expect(error).toBeNull();
      expect(data).toHaveLength(1);
    ...
    it("should reject anon users from deleting patient rates", async () => {
      const { error } = await anonSupabase.from("patient_rates").delete().eq("id", testPatientRate.id);
      expect(error).not.toBeNull();
    });
  });
});
